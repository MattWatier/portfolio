...
//indexjson.php
// Request to Mysql;
$collection = query_full_array("SELECT zp_albums.folder,zp_albums.title, zp_images.filename, zp_images.title, zp_images.date FROM zp_images LEFT JOIN zp_albums ON zp_images.albumid=zp_albums.id;");

//SELECT LEFT( zp_images.date , 4 ) AS Year,LEFT(zp_albums.folder,(LOCATE('/',zp_albums.folder)-1)) AS Parent, zp_albums.title, zp_images.filename, zp_images.title,  zp_images.width,  zp_images.height FROM zp_images LEFT JOIN zp_albums ON zp_images.albumid=zp_albums.id;





//SELECT LEFT( zp_images.date , 4 ) AS Year,LEFT(zp_albums.folder,(LOCATE('/',zp_albums.folder)-1)) AS Parent, zp_albums.title, zp_images.filename, zp_images.title,  zp_images.width,  zp_images.height FROM zp_images LEFT JOIN zp_albums ON zp_images.albumid=zp_albums.id WHERE LEFT(zp_albums.folder,(LOCATE('/',zp_albums.folder)-1)) = 'Paper';
