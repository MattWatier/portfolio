...
function drawColorChart(chartID, dataSet,dataSetName, selectString){
  ...
  var color = d3.scale.ordinal()
    .domain(["_color-black","_color-grey","_color-white","_color-pink","_color-red","_color-orange","_color-brown","_color-yellow", "_color-green","_color-blue","_color-purple"])
    .range(["#222222","#888888","#dfdfdf","#EF3368","#EB1313","#E75516","#5D3A18" ,"#FDE93A" ,"#5DD245","#448BD2", "#6E4ACB"]);
  var xc = d3.scale.linear()
      .domain([0,1716])
      .range([0, colorChart.w-500]);  
      console.log(xc(1716));   
  var xd = d3.scale.linear()
      .domain([ 
        d3.min(dSet,function(d){return d.date}),
        d3.max(dSet,function(d){return d.date})
          ])
      .range([0, colorChart.w-500]);            

  var color_chart_svg = d3.select(selectString).append("svg")
      .attr("class", function(){return "color"+chartID;})
      .attr("width", colorChart.w)
      .attr("height", colorChart.h)
      .append("g")
      .attr("transform","translate(" + 250 + "," + 250 + ")");

  var circles = color_chart_svg.selectAll(".circles")
      .data(dataSet)
      .enter().append("g")
  ...
  circles.append("circle") 
      .attr("cy",200)
      .attr("cx",function(d){ return xd(d.date)+ colorChart.w/2; } )
      .attr("data-role",function(d){ return d.name; })
      .attr("r", 3)
  ...
  circles.append("circle")
      .attr("cy",100)
      .attr("cx", function(d){ return xc(d.id) + 0; } )
      .attr("data-role",function(d){ return d.name; })
      .attr("r", 3)
      .style("fill", function(d) { return color(d.name); })
      .style("fill-opacity",0.75);

  var curves = circles.append("svg:line")
      .attr("x", function(d){ return xd(d.date)+ colorChart.w; })
      .attr("y", function(d){ return xc(d.id) + 0; })
      .attr("y1", 200)
      .attr("y2", 100)
...
var bchart_svg = d3.select(selectString).append("svg")
  .attr({
    class:function(){return "bar"+chartID;},
    width:barChart.w,
    height:barChart.h,
...
var bchart = bchart_svg.selectAll(".bar")
 ...
 bchart.append("svg:a")
    ...
    .attr({
      ...
      y:function(d){return   barChart.height  - y( d.count ); },
  ...
  bchart.append("text")
    ...
    .attr({ 
      ...
      y: function(d){return   barChart.height  - y( d.count ) + 55; },
      width: x.rangeBand(),
      fill: "#ffffff",
      "font-size":"40px",
      "font-wieght": 900
  ...
  bchart.append("text")
    ...
    class: "label",
    x: function(d,i){ return x( i ) + 10 },
    y: function(d){return   barChart.height  - y( d.count ) + 20; },
    width: x.rangeBand(),
    fill: "#ffffff",
    "fontsize":"10px"});
...
function drawColorWheel( selectData , dataSet, selectString,dimensions){
  ...
  var color = d3.scale.ordinal();
    color.domain( ["_color-black","_color-grey","_color-white","_color-pink","_color-red","_color-orange","_color-brown","_color-yellow", "_color-green","_color-blue","_color-purple"]); 
  color.range( ["#222222","#888888","#dfdfdf","#EF3368","#EB1313","#E75516","#5D3A18" ,"#FDE93A" ,"#5DD245","#448BD2", "#6E4ACB"                           ]);
 var pie = d3.layout.pie().sort(null).value(function(d){return d.count});
var arc = d3.svg.arc()
